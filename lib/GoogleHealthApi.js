...
class GoogleHealthApi {
  ...
  /**
   * Scopes for the app. Read-only by default; weight logging needs the
   * read/write health metrics scope.
   */
  static scopes({ allowWrite = false } = {}) {
    return [
      `${SCOPE_PREFIX}activity_and_fitness.readonly`,
      allowWrite
        ? `${SCOPE_PREFIX}health_metrics_and_measurements`
  ...
  /**
   * Authorization URL. Homey's OAuth2 callback appends redirect_uri and state.
  ...
  async request(method, path, { query, body } = {}) {
    ...
    if (!res.ok) {
      ...
      throw new GoogleHealthApiError(`Google Health API error: ${message}`, { statusCode: res.status });
    }
