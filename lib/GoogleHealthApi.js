 ...
 *   GET  /users/me/identity                                 — user info
 *
 * Read-only: the app never writes data points back to Google Health.
 *
...
class GoogleHealthApi {
  ...
  /**
   * Scopes for the app. Read-only throughout; cardiac (ECG + irregular-rhythm)
   * has its own dedicated readonly scopes and is requested only when opted in.
   */
  static scopes({ allowCardiac = false } = {}) {
    const scopes = [
    ...
    if (allowCardiac) {
      scopes.push(`${SCOPE_PREFIX}ecg.readonly`);
      scopes.push(`${SCOPE_PREFIX}irn.readonly`);
    }
    return scopes;
  }
  ...
  async request(method, path, { query, body } = {}) {
    ...
    const doFetch = async accessToken => {
      ...
      return fetch(url, {
        ...
        // A stalled endpoint must not hold the sync mutex for minutes
        signal: AbortSignal.timeout(30_000),
      });
    };
    ...
    if (!res.ok) {
      ...
      // 403 "API disabled" means the whole Cloud project is broken.
      let code = null;
      if (res.status === 403) {
        if (/scope/i.test(message)) code = 'missing_scope';
        else if (/disabled|has not been used/i.test(message)) code = 'api_disabled';
      }
  ...
  /**
   * List data points across pages (follows nextPageToken).
   * @returns {Promise<Array>} raw dataPoints, newest first
   */
  async listAll(type, { filter, pageSize = 100, maxPages = 5 } = {}) {
    const all = [];
    let pageToken = null;
    for (let page = 0; page < maxPages; page++) {
      ...
      const res = await this.request('GET', `/users/me/dataTypes/${type}/dataPoints`, { query });
      if (Array.isArray(res.dataPoints)) all.push(...res.dataPoints);
  ...
  /** Civil date (YYYY-MM-DD) of a raw rollup point, or null. */
  static rollupDate(point) {
    const civil = point && point.civilStartTime && point.civilStartTime.date;
  ...
  /**
   * Like firstNumber, but searches nested objects up to `depth` levels deep —
   * for types whose value lives inside a sub-object (e.g. nutrition nutrients).
   */
  static deepNumber(point, candidates = [], depth = 2) {
    const values = GoogleHealthApi.valueObject(point);
    if (!values) return null;
    const search = (obj, d) => {
      for (const field of candidates) {
    ...
    return search(values, depth);
  }

  /**
   * Parse a protobuf Duration into seconds. The JSON encoding is a string with
   * a trailing "s", optionally fractional, e.g. "50880s" or "50880.500s".
   * Also accepts a plain number. Returns null if not parseable.
   */
  static durationSeconds(value) {
    if (value === undefined || value === null) return null;
    if (typeof value === 'number') return Number.isFinite(value) ? value : null;
    if (typeof value !== 'string') return null;
    const num = Number(value.endsWith('s') ? value.slice(0, -1) : value);
    return Number.isFinite(num) ? num : null;
  }
